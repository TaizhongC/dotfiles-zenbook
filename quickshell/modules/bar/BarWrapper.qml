...
Scope {
    ...
    Loader {
        id: wallpaperLoader
        source: "../wallpaper/WallpaperWindow.qml"
        asynchronous: true
        property var wallpaper: item
    }

    // Expose the launcher to compositor keybindings. `qs ipc call shell
    // toggleLauncher` is used by Super+Space, so no separate launcher process
    // is needed.
    IpcHandler {
        target: "shell"

        function toggleLauncher(): void {
            const launcher = launcherLoader.item
            if (!launcher)
                return
            ...
            if (launcher.shouldShow) {
                if (controlCenterLoader.item)
                    controlCenterLoader.item.shouldShow = false
                if (sidebarLoader.item)
                    sidebarLoader.item.shouldShow = false
                if (dashboardLoader.item)
                    dashboardLoader.item.shouldShow = false
            }
        }
    }
            ...
            Loader {
                ...
                onStatusChanged: {
                    if (status === Loader.Ready) {
                        ...
                        item.wallpaper = Qt.binding(() => wallpaperLoader.item)
                    }
                }
            }
        }
