...
Singleton {
    ...
    // pywal replaces its cache atomically; periodic reload also catches that
    // replacement on filesystems where a file watch only sees the old inode.
    Timer { interval: 500; running: true; repeat: true; onTriggered: pywalFile.reload() }
}
