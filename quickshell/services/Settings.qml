...
Singleton {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    root.launcherHistory = settings.launcherHistory ?? []
                } catch(e) {
    ...
    function doSaveSettings() {
        const settings = {
            ...
            ,launcherHistory: root.launcherHistory
        }
    ...
    onLauncherHistoryChanged: saveSettings()

    function rememberLauncherSearch(query: string): void {
        const text = query.trim()
        if (!text.length)
            return
        const previous = root.launcherHistory ?? []
        const existing = previous.find(item => item.query === text)
        const count = (existing?.count ?? 0) + 1
        root.launcherHistory = [{ query: text, count: count, lastUsed: Date.now() }]
            .concat(previous.filter(item => item.query !== text))
